...
```{r}
## Summary by species : Abundance, cpue, and relative abundance
dd <- d %>% group_by(family, genus, species) %>% summarise(Abondance = sum(n), cpue = round(sum(cpue),3), ra = round((sum(n)*100/sum(d$n)),3))
```
