...
# test
aovq(cpue ~ topography, tau=c(0.5, 0.75, 0.9), data=dd, test="anowar", R=1000)
...
```{r}
# test
aovq(cpue ~ region, tau=c(0.5, 0.75, 0.9), data=dd, test="anowar", R=1000)
...
p_val <- d_abund %>% group_by(sp) %>% do(tidy((aovq(cpue ~ topography, tau=c(0.5, 0.75, 0.9), data=., test="rank"))$table$pvalue))
tau_val <- d_abund %>% group_by(sp) %>% do(tidy((aovq(cpue ~ topography, tau=c(0.5, 0.75, 0.9), data=., test="rank"))$table$tau))
...
p_val <- d_abund %>% group_by(sp) %>% do(tidy((aovq(cpue ~ region, tau=c(0.5, 0.75, 0.9), data=., test="rank"))$table$pvalue))
tau_val <- d_abund %>% group_by(sp) %>% do(tidy((aovq(cpue ~ region, tau=c(0.5, 0.75, 0.9), data=., test="rank"))$table$tau))
