...
Excluding those extreme events and focusing on the more common, low (CPUE < 10), catches, allows to highlight that some sites yield better catches (`r head(dd_summary$site, 4)`) and other are consistently low (`r tail(dd_summary$site, 4)`). NB: Keeping only low catches only eliminates `r nrow(filter(dd, cpue >= 10))` records out of `r nrow(dd)`.
...
Those differences are reflected by global and pairwise statistical tests. Abundances are significantly different between sites at all levels (low, mid, high, and very high), albeit more so for mid to low catches. In terms of median catches, Bastia is often below and Port-Cros and La Ciotat often above the others.

```{r tests_among_sites, fig.height=6}
aovq(cpue ~ site, tau=c(0.25, 0.5, 0.75, 0.9), data=dd)

pw <- suppressMessages(pairwise.aovq(dd$cpue, dd$site, tau=c(0.25, 0.5, 0.75, 0.9), R=100))
# plot
...
Use quantiles and test the difference between sites/regions for high catches, medium catches and low catches only. investigate quantreg() with a discrete explanatory variable.

```{r}
## Quantile regression for high catches, medium catches and low catches with discrete variable : comparison between sites
p_val <- d_abund %>% group_by(sp) %>% do(aovq(cpue ~ topography, tau=c(0.25, 0.5, 0.75, 0.9), data=., test="rank")$table[,c("tau", "pvalue")]) %>% spread(key=tau, value=pvalue)


# Pour toutes les espèces, des différences entre les sites, mais quels sont les sites qui diffèrent ??
#TODO compute pairwise.aovq
```
...
# TODO use the ready-made function in from test_specaccum
# TODO perform tests of differences among sites at various effort levels
...
hdw <- hclust(dist_matrix, method="ward.D2")
dend <- as.dendrogram(hdw)
dend %>% set("branches_k_color", k = 4) %>% set("labels_col", k=4) %>% plot(las=1, ylab="Heigh")
```
