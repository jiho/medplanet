...
```{r}
# sort in decreasing order of catches to dosplay stats, below
dd_summary <- arrange(dd_summary, median)
```

Excluding those extreme events and focusing on the more common, low (CPUE < 10), catches, allows to highlight that some sites yield better catches (`r head(dd_summary$site, 4)`) and other are consistently low (`r tail(dd_summary$site, 4)`). NB: Keeping only low catches eliminates `r nrow(filter(dd, cpue >= 10))` records from `r nrow(dd)`.
...
aovq(cpue ~ site, tau=c(0.5, 0.75, 0.9), data=dd)
suppressWarnings(pw <- pairwise.wilcox.test(dd$cpue, dd$site))
# TODO replace with pairwise aovq
# plot
pw <- melt(pw$p.value)
...
# test
aovq(cpue ~ topography, tau=c(0.5, 0.75, 0.9), data=dd)
...
```{r}
# test
aovq(cpue ~ region, tau=c(0.5, 0.75, 0.9), data=dd)
