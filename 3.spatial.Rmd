...
hdw <- hclust(dist_matrix, method="ward.D2", vfont = NULL, col = part$group)
plot(hdw, vfont = NULL, col = part$group)


hdw2 <- dendro_data(hdw, type="triangle")
...
dendr    <- dendro_data(hdw, type="triangle") # convert for ggplot
clust    <- cutree(hdw,k=6)                    # find 2 clusters
...
ggplot() + 
   ...
   coord_flip() +
    theme(axis.line.y = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        panel.grid = element_blank())
ggsave("figures/dendrogram_sites.pdf", width = 6, height = 5)
