...
```{r}
# Compute a Contingency table to find rare and common species
t <- unique(select(d, site, species))
t <- as.data.frame.matrix(t(table(t$site, t$species)))
t$sum <- apply(t,1,sum)
```

Among the `r length(unique(na.omit(d$species)))` identified species, `r length(rownames(t[which(t$sum == 1),]))` are catched only in one site (not necessary the same) and `r length(rownames(t[which(t$sum >= 10),]))` are catched at least in 10 sites.

```{r}
#Species found in only one site
scarce <- t[which(t$sum == 1),]
scarce_species <- data.frame(species = as.character(rownames(scarce)))
list_scarce <- right_join(d, scarce_species, by="species")
list_scarce <- arrange(unique(select(list_scarce, site, species)), site)
```

```{r}
#Species found in several site
cosmo <- t[which(t$sum > 7),]
cosmo_species <- data.frame(species = as.character(rownames(cosmo)))
list_cosmo <- right_join(d, cosmo_species,by="species")
list_cosmo <- unique(select(list_cosmo, species))
```
...
dd_test$region <- NA
dd_test$topography <- NA

#Give a region to each site
dd_test$region[which(dd_test$site=="Port Vendres")]<-"west_rhone"
dd_test$region[which(dd_test$site=="Leucate")]<-"west_rhone"
dd_test$region[which(dd_test$site=="Agde")]<-"west_rhone"
dd_test$region[which(dd_test$site=="Carry")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Marseille")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Cassis")]<-"east_rhone"
dd_test$region[which(dd_test$site=="La Ciotat")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Les Embiez")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Port-Cros")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Villefranche")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Bastia")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Saint Florent")]<-"east_rhone"
dd_test$region[which(dd_test$site=="Bonifacio")]<-"east_rhone"


#Give a topography to each site
# Gulf of lion plateau and deep coatal waters of PACA
dd_test$topography[which(dd_test$site=="Port Vendres")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Leucate")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Agde")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Carry")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Marseille")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Cassis")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="La Ciotat")]<-"gulf_lion"
dd_test$topography[which(dd_test$site=="Les Embiez")]<-"PACA"
dd_test$topography[which(dd_test$site=="Port-Cros")]<-"PACA"
dd_test$topography[which(dd_test$site=="Villefranche")]<-"PACA"
dd_test$topography[which(dd_test$site=="Bastia")]<-"PACA"
dd_test$topography[which(dd_test$site=="Saint Florent")]<-"PACA"
dd_test$topography[which(dd_test$site=="Bonifacio")]<-"PACA"
...
dd_topography <- dd_test %>% group_by(region, species) %>% summarise(mean = mean(cpue), sd=sd(cpue))
ggplot(dd_region, aes(x = species, y = mean))+ geom_errorbar(aes(ymin=mean, ymax=mean+sd), width=0.2)+geom_bar(stat='identity') + facet_grid(~topography) +theme(axis.text.x=element_text(angle=25, hjust=1))+ylim(0,6)

#Test differences between site and substrate
wilcox.test(cpue~substrate, data=dd_test) # but unbalanced factors
kruskal.test(cpue~site, data=dd_test)

#Comparison for cpue of each species
...
syn <- syn %>% group_by(year, site, species) %>% summarise(cpue = mean(cpue), date = mean(date), yweek=mean(yweek)) %>% ungroup()

syn_year_site <- syn %>% group_by(site, species) %>% summarise(ref_date = mean(date), ref_yweek=mean(yweek))%>%ungroup()
syn_site <- syn %>% group_by(year, species) %>% summarise(ref_date = mean(date), ref_yweek=mean(yweek))%>%ungroup()
syn_year <- syn %>% group_by(species) %>% summarise(ref_date = mean(date), ref_yweek=mean(yweek)) %>% ungroup()

# Time lag between site, for each year and each species
lag <- left_join(syn_site,syn, by = c("year","species"))
lag <- lag %>% group_by(species,year,site, ref_date, ref_yweek, date, yweek) %>% summarise(lag_day = date-ref_date, lag_week = yweek-ref_yweek)%>%ungroup()

# Time lag between site, for each species
...
lag <- lag %>% group_by(species,site, ref_date, ref_yweek, date, yweek) %>% summarise(lag_day = date-ref_date, lag_week = yweek-ref_yweek)%>%ungroup()
...
lag <- lag %>% group_by(species, year, ref_date, ref_yweek,date, yweek) %>% summarise(lag_week = yweek-ref_yweek)
