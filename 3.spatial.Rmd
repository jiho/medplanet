...
```{r}
dd_div <- left_join(select(d0, date, year, site, species, cpue), select(sites, site, topography, region))

div <- dd_div %>% filter(cpue!=0) %>% group_by(date, year, site) %>% summarise(shannon = diversity(cpue, index="shannon", 
...
div %>% group_by(region) %>% summarise(shannon_mean = str_c(round(mean(shannon),2), "±", round(sd(shannon),1)), 
shannon_median = str_c(round(median(shannon),2),"±",round(mad(shannon),1)), pielou_mean = str_c(round(mean(pielou, 
na.rm=T),2), "±", round(sd(pielou, na.rm=T),1)), pielou_median = str_c(round(median(pielou, na.rm=T),2),"±",round(mad
(pielou, na.rm=T),1)))
