...
# ggsave("figures/Fig3_CPUE_per_sites.pdf", width=6, height=4)
```
...
riv <- betadisper(m, group=sites$rhone)
anova(riv)
plot(riv)
...
cut <- 6 # ward.D2 cut at 5, but cutting at 6 enables to discriminate Vlfr from Corsica on Axe 4
...
ggplot() + 
   ...
   geom_hline(aes(yintercept=2.85), linetype=2)+
...
# Save plot
# ggsave("figures/dendrogram_sites.pdf", width = 10, height = 3)


# Try to find cutoff with some criterion
NbClust(data=ddm, diss=dist_matrix, distance=NULL, min.nc=2, max.nc=8, method="ward.D2", index=c("kl", "ch", "cindex", "db", "silhouette", "duda", "pseudot2", "beale", "gap"))
...
glance(wilcox.test(filter(dp, rhone=="West of Rhone")$lag, filter(dp, rhone=="East of Rhone")$lag, alternative = "two.sided")) %>% select(statistic, p.value)
#   statistic p.value
# 1      1130  0.0028
