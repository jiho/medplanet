...
```{r}
# mean CPUE per week, site, and species for common species
dd <- d0 %>% filter(year !=2016, species %in% common_species) %>% group_by(date, yweek, yday, year, site, species) %>% summarise(cpue=mean(cpue))
...
syn <- dd %>% group_by(year, site, species) %>% summarise(cpue = mean(cpue), date = mean(date), yweek=mean(yweek)) %>% ungroup() # mean because some species have twice the same max abundance
...
syn_species <- syn %>% group_by(species) %>% summarise(ref_date = mean(date), ref_yweek=mean(yweek), earliest = min(yweek), lastest = max(yweek)) %>% ungroup() # ref all sites and year includes
...
# Barplot of mean cpue
dd_site <- dd_test %>% group_by(site, species) %>% summarise(mean = mean(cpue), sd=sd(cpue))
dd_site$species <- abbrev_sp(dd_site$species, n=4)

ggplot(dd_site, aes(x = species, y = mean))+ geom_errorbar(aes(ymin=mean, ymax=mean+sd), width=0.2)+geom_bar(stat='identity') + facet_wrap(~site, ncol=3) +theme(axis.text.x=element_text(angle=25, hjust=1))+ylim(0,30) 
```

```{r}
as.data.frame(syn_species)
```

Leucate : `r sort(unique(syn_site$species[which(syn_site$site=="Leucate")]))`

Port-Vendres : `r sort(unique(syn_site$species[which(syn_site$site=="Port Vendres")]))`

Agde : `r sort(unique(syn_site$species[which(syn_site$site=="Agde")]))`

Carry : `r sort(unique(syn_site$species[which(syn_site$site=="Carry")]))`

Marseille : `r sort(unique(syn_site$species[which(syn_site$site=="Marseille")]))`

Cassis : `r sort(unique(syn_site$species[which(syn_site$site=="Cassis")]))`

La Ciotat : `r sort(unique(syn_site$species[which(syn_site$site=="La Ciotat")]))`

Les Embiez : `r sort(unique(syn_site$species[which(syn_site$site=="Les Embiez")]))`

Port-Cros : `r sort(unique(syn_site$species[which(syn_site$site=="Port-Cros")]))`

Villefranche : `r sort(unique(syn_site$species[which(syn_site$site=="Villefranche")]))`

Saint Florent : `r sort(unique(syn_site$species[which(syn_site$site=="Saint Florent")]))`

Bastia : `r sort(unique(syn_site$species[which(syn_site$site=="Bastia")]))`

Bonifacio : `r sort(unique(syn_site$species[which(syn_site$site=="Bonifacio")]))`
