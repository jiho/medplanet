...
## Test with Anova + Tukey => Hypothèses non vérifiées même après transformation log
#dd$cpue_log <- log(dd$cpue+1)
anova <- aov(cpue~site, data=dd)
shapiro.test(residuals(anova)) 
bartlett.test(residuals(anova)~dd$site)

## Test with non parametric test
kruskal.test(cpue ~ site, data=dd)
pairwise.wilcox.test(dd$cpue, dd$site, p.adjust.method = "bonferroni")
```

Same on a map with q75 in blue, median in green, and q25 in yellow.
...
TODO test differences among sites (ANOVA + Tuckey or Kuskal + pairwise wilcox).
...
The same tests can be performed per species, to detect if a given species is caught more in the west or in the east. The following tables display the mean±SD of the CPUE for each species in each region, together with the statistic and *p*-value of a Kruskal-Wallis test between regions.
...
test <- d_abund %>% group_by(sp) %>% do(glance(kruskal.test(cpue ~ topography, data=.)))

# make a nice table with both
left_join(descriptive_stats, select(test, sp, statistic, p.value), by="sp") %>% ungroup()
```

```{r}
# and the corresponding plot...
# TODO use violins + log transform, as in taxonomic composition
ggplot(d_abund, aes(x=sp, y=cpue))+ geom_violin(na.rm=T) + geom_point(alpha=0.2, size=1) + scale_y_continuous(trans="log10") + facet_wrap(~topography) + theme(axis.text.x=element_text(angle=35, hjust=1))
```
...
test <- d_abund %>% group_by(sp) %>% do(glance(kruskal.test(cpue ~ region, data=.)))
...
# and the corresponding plot...
# TODO use violins + log transform, as in taxonomic composition
ggplot(d_abund, aes(x=sp, y=cpue))+ geom_violin(na.rm=T) + geom_point(alpha=0.2, size=1) + scale_y_continuous(trans="log10") + facet_wrap(~region) + theme(axis.text.x=element_text(angle=35, hjust=1))
```
...
```{r}
## No solution for quantile regression with discrete variable. Maybe quantile poisson regression ?
## But i don't understand all
library(quantreg)

high_catches <- d_abund[which(d_abund$cpue >= 12),] 
mid_catches <- d_abund[which(d_abund$cpue < 12 & d_abund$cpue>= 2),]
low_catches <- d_abund[which(d_abund$cpue < 2),]

test <- low_catches %>% group_by(sp) %>% do(glance(kruskal.test(cpue ~ region, data=.)))
```
...
## Number of nights 
dd_night <- d %>% group_by(site) %>% summarise(nb_night=length(unique(date)))
...
TODO compute effort (nb of nights deployed) vs nb of species curves to see if this is just related to differing effort or if there is a true difference in diversity among sites. This can be compared to theoretical assumptions (see species-area relationships).

```{r}
## Plot species-nigths accumulation curves

##TODO : Try to compute a for loop for each site and paste all data.frames to plot all
## curves in facet_wrap

dd <- d %>% group_by(date,site,species) %>% summarise(cpue=sum(cpue))
dd <- filter(dd, !is.na(species))

vilf <- filter(dd, site=="Villefranche")
vilf <- cast(vilf, date~species, sum)
vilf <- specaccum(vilf[,2:ncol(vilf)], method="exact")
vilf <- data.frame(nights=vilf$sites, richness = vilf$richness)
vilf$site <- "Villefranche"

ciot <- filter(dd, site=="La Ciotat")
ciot <- cast(ciot, date~species, sum)
ciot <- specaccum(ciot[,2:ncol(ciot)], method="exact")
ciot <- data.frame(nights=ciot$sites, richness = ciot$richness)
ciot$site <- "La Ciotat"

data <- rbind(vilf, ciot)

ggplot(data) + geom_line(aes(nights, richness)) + facet_wrap(~site)
```
...
# TODO add cutoff based on some criterion and plot groups as colors, on the dendrogram or/and on the CA output
```
...
TODO test with t.test or wilcox.test (one sample)
...
test <- lag %>% group_by(site) %>% do(glance(wilcox.test(.$lag_week,mu = 0,alternative = "two.sided")))
