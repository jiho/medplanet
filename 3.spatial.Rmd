...
lag <- lag %>% group_by(species,site, ref_date, ref_yweek, date, yweek) %>% summarise(lag_week = yweek-ref_yweek)%>%ungroup()
...
lag <- lag %>% group_by(species, year, ref_date, ref_yweek,date, yweek) %>% summarise( lag_week = yweek-ref_yweek) %>% ungroup()
lag <- lag %>% arrange(species, lag_week)
