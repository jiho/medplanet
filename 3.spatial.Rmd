...
d0$site <- factor(d0$site, levels=sites_ordered)


#!!!!!!!! IF FOCUSING ALL ANALYSIS ON SUMMER MONTHS ONLY
# Keep only summer months for comparison
d0 <- filter(d0, month%in%c(5:9))
head(d0)

d <- filter(d, month%in%c(5:9))
head(d0)
...
summary_effort <- effort %>% group_by(site) %>% summarise(begin=min(date), end=max(date, na.rm=T), n_nights_sampled=length(unique(date)), `n_CARE_fished`=sum(n_gear, na.rm=T)) %>% ungroup()
summary_effort$site <- factor(summary_effort$site, levels=levels(effort$site))
summary_effort
dd$site_effort <- factor(dd$site, levels=levels(dd$site), labels=str_c(levels(dd$site), " (n=", summary_effort$n_CARE_fished, ")"))


ggplot(dd) +
  geom_boxplot(aes(x=site_effort, y=cpue), na.rm=T) +
  geom_quantiled(mapping=aes(x=dd$site_effort, y=dd$cpue), quantiles=0.9, colour="black", size=3, shape=0) +
  scale_x_discrete(limits=rev(levels(dd$site_effort))) + scale_y_continuous("CPUE") +
  coord_flip(ylim=c(0, 18.5)) + theme(axis.title.y=element_blank())
# Save plot
# ggsave("figures/Fig3_CPUE_per_sites.png", width=6, height=4)
# ggsave("figures/Fig3_CPUE_per_sites_SUMMER.pdf", width=6, height=4)
...
```{r among_sites}
aovq(cpue ~ site_effort, tau=c(0.25, 0.5, 0.75, 0.9), data=dd, R=1000)
```

In terms of pairwise differences:

- q=0.25: Saint-Florent is higher than others
- q=0.5: Marseille and Bastia are lower, Villefranche and La Ciotat are higher
- q=0.75: Bastia is lower
- q=0.9: Villefranche is higher than others; probably because it is more easily estimated (more effort).
- wilcox: Marseille and Bastia are lower; La Ciotat, Port-Cros and St-Florent are higher; Carry is often barely signif and would be similar to Marseille.

Also, Saint Florent, Bonifacio, Villefranche, Port-Cros, Les Embiez, La Ciotat, Cassis are never significantly different from each other.

NB: The p-values seem quite sensitive and may change form run to run. We need to reason in terms of global behaviour and trend of the p-value. 

```{r pairwise_among_sites}
# compute pairwise AOV
pwa <- suppressMessages(pairwise.aovq(dd$cpue, dd$site, tau=c(0.25, 0.5, 0.75, 0.9), R=2000))
# and pairwise wilcoxon tests
pww <- suppressWarnings(pairwise.wilcox.test(dd$cpue, dd$site, p.adjust.method="holm"))
pwa$wilcox <- pww
```

```{r plot_among_sites, fig.height=8}
# reformat pairwise p-value tables as data.frame
pw <- ldply(pwa, tidy)
pw$group1 <- factor(pw$group1, levels=sites$site)
pw$group2 <- factor(pw$group2, levels=sites$site)
# label signficant tests
pw$signif <- pw$p.value <= 0.05

# compute the actual values of the quantiles
q <- dd %>% group_by(site) %>% 
  summarise(
    `0.25`=quantile(cpue, 0.25),
    `0.5` =quantile(cpue, 0.5),
    `0.75`=quantile(cpue, 0.75),
    `0.9` =quantile(cpue, 0.9)
  ) %>% 
  gather(key=".id", value="value", -site)
# and the values of the pairwise difference in quantile
pw$q.diff <- q$value[match(pw$group1, q$site)] - q$value[match(pw$group2, q$site)]

# make the matrices symmetric, with difference in quantiles at the top and p-values at the bottom
pw <- bind_rows(
  pw %>% mutate(q.diff=NA),
  dplyr::rename(pw, group1=group2, group2=group1) %>% mutate(p.value=NA)
)

# plot
ggplot(pw, aes(group1, group2)) +
  # plot difference in quantiles
  geom_point(aes(colour=q.diff), na.rm=T, size=5, shape=15) +
  # plot p-value
  geom_point(aes(fill=p.value), na.rm=T, size=6, shape=22, stroke=0) +
  # mark significant differences
  geom_point(colour="red", na.rm=T, size=5, shape=22, data=filter(pw, signif)) +
  # reformat
  facet_wrap(~.id, ncol=2) + coord_fixed() +
  scale_colour_gradient2("Quantile diff\n(row-col)", na.value=NA, low="#7b3294", mid="#f7f7f7", high="#008837") +
  scale_fill_distiller("p-value", palette="PuBu", direction=1, na.value=NA) +
  theme(
...
ggsave("figures/pairwise_test_cpue_summer.pdf")

```

Overall, it seems Bonifacio, Cassis, and Carry are difficult to evaluate (low sampling effort); Bastia, Marseille, Agde, Port-Vendres, and Leucate yield low catches; Saint-Florent, Villefranche, Port-Cros, Les Embiez, La Ciotat yield high catches.
...
# SUMMER
# Model 1: cpue ~ topography
# Model 2: cpue ~ 1
#    tau Df Resid Df F value   Pr(>F)    
# 1 0.25  1      758    1.56  0.00561 ** 
# 2 0.50  1      758    9.05 0.000013 ***
# 3 0.75  1      758   27.72 0.000077 ***
# 4 0.90  1      758   79.30  0.00035 ***
...
#   topography    mean    sd   q75   q90
  # <fct>        <dbl> <dbl> <dbl> <dbl>
# 1 Gulf of Lion  2.41  5.89     2  5.44
# 2 Ligurian sea  8.61 48.7      4 13.9 
...
# Model 2: cpue ~ 1
#    tau Df Resid Df F value   Pr(>F)    
# 1 0.25  1      758    0.06     0.58    
# 2 0.50  1      758    1.63     0.10 .  
# 3 0.75  1      758   19.62 0.000023 ***
# 4 0.90  1      758   86.79 0.000029 ***
...
colnames(descriptive_stats) <- c("Species", "CPUE in GoL (mean±SD)", "CPUE in LS (mean±SD)", "q25", "q50", "q75", "q90")
...
#    Species          `CPUE in GoL (mean±SD)` `CPUE in LS (mean±SD)`     q25     q50   q75     q90
#    <chr>            <chr>                   <chr>                    <dbl>   <dbl> <dbl>   <dbl>
#  1 A. hepsetus      0±0.1                   0.12±1.7               0.00627 0.712   0.102 0.00260
#  2 B. boops         0±0                     0.19±1.3               0.0210  0.460   0.355 0.286  
#  3 C. chromis       0.45±3.2                0.78±4.8               0.001   0.0364  0.001 0.001  
#  4 D. annularis     0.15±1.2                0.31±2.2               0.145   0.622   0.619 0.651  
#  5 G. mediterraneus 0.05±0.2                0.04±0.3               0.0210  0.666   0.102 0.001  
#  6 M. surmuletus    0.21±1.1                0.29±2.7               0.00627 0.902   0.619 0.712  
#  7 O. melanura      0±0                     0.26±2.6               0.0143  0.460   0.102 0.0364 
#  8 P. acarne        0.08±0.5                0.13±0.8               0.0143  0.498   0.594 0.0347 
#  9 P. bogaraveo     0.16±1                  0±0                    0.374   0.00682 0.001 0.001  
# 10 P. pilicornis    0.23±1                  0.11±0.9               0.455   0.001   0.001 0.001  
# 11 S. salpa         0.07±0.7                0.14±0.8               0.00954 0.805   0.522 0.203  
# 12 S. smaris        0.29±2.7                0.95±6.7               0.803   0.712   0.102 0.001  
...
# make a nice table with both
descriptive_stats <- left_join(descriptive_stats, tests, by="sp") %>% ungroup()
colnames(descriptive_stats) <- c("Species", "CPUE in W (mean±SD)", "CPUE in LS (mean±SD)", "q25", "q50", "q75", "q90")
descriptive_stats %>% mutate(q25=if_else(q25>0.001, q25, 0.001), q50=if_else(q50>0.001, q50, 0.001), q75=if_else(q75>0.001, q75, 0.001), q90=if_else(q90>0.001, q90, 0.001))
#    Species          `CPUE in W (mean±SD)` `CPUE in LS (mean±SD)`     q25     q50     q75     q90
#    <chr>            <chr>                 <chr>                    <dbl>   <dbl>   <dbl>   <dbl>
#  1 A. hepsetus      0±0                   0.09±1.5               0.001   0.001   0.001   0.262  
#  2 B. boops         0±0                   0.15±1.2               0.001   0.001   0.001   0.944  
#  3 C. chromis       0.02±0.1              0.85±4.8               0.00108 0.00166 0.00321 0.001  
#  4 D. annularis     0.02±0.3              0.32±2.1               0.001   0.001   0.115   0.001  
#  5 G. mediterraneus 0.04±0.1              0.05±0.3               0.001   0.001   0.001   0.001  
#  6 M. surmuletus    0.34±1.5              0.23±2.3               0.001   0.001   0.001   0.00142
#  7 O. melanura      0±0                   0.2±2.3                0.001   0.001   0.001   0.591  
#  8 P. acarne        0.12±0.6              0.11±0.7               0.001   0.001   0.001   0.944  
#  9 P. bogaraveo     0.16±1                0.04±0.4               0.001   0.001   0.001   0.001  
# 10 P. pilicornis    0.33±1.2              0.1±0.8                0.001   0.001   0.001   0.001  
# 11 S. salpa         0.1±0.9               0.11±0.7               0.001   0.001   0.001   0.273  
# 12 S. smaris        0±0                   0.89±6.2               0.001   0.001   0.0689  0.001  
...
Species richness vs effort (nb of nights fished or nb of CARE deployed) curves are useful to know if this is just related to differing effort or if there is a true difference in diversity among sites. Here, the curves take a long time to begin to saturate (>150 nights) and never really fully do. 
...
```{r plot_specaccum, fig.height=4.5}
# and some plots
differentiating_scales <- list(
  scale_linetype_manual("Sites", values=rep(c("solid", "22", "11"), times=5)),
  scale_fill_discrete("Sites", h=c(0,280)+15), scale_colour_discrete("Sites", h=c(0,280)+15),
...
ggplot(sp_per_night) +
  # geom_vline(aes(xintercept=20) , colour="black", size=0.25) +
  # geom_vline(aes(xintercept=50) , colour="black", size=0.25) +
  # geom_vline(aes(xintercept=150), colour="black", size=0.25) +
  geom_ribbon(aes(x=nights, ymin=richness-sd, ymax=richness+sd, fill=site), alpha=0.2) +
  geom_path(aes(x=nights, y=richness, colour=site, linetype=site)) +
  theme_light() + differentiating_scales +
  labs(x="Nights sampled", y="Species richness")

# Save plot
ggsave("figures/Fig4_species_richness.pdf", width = 6, height = 5.5)
ggsave("figures/Fig4_species_richness_SUMMER.pdf", width = 6, height = 5.5)
...
pairwise.specaccum.test <- function(comm, g, effort=c(20, 50, 150), nperm=1000, p.adjust.method="holm") {
...
ps <- pairwise.specaccum.test(comm=select(dw, -date, -site, -n_gear), g=dw$site, effort=c(20, 50, 90))
```
...
ggsave("figures_specaccum_SUMMER.pdf")
...
div %>% group_by(site) %>% do(div_stats(.)) %>% as.data.frame()
...
# SUMMER 
#KS significant at p=0.05 but pairwise could not identify site
# 	Kruskal-Wallis rank sum test
# 
# data:  pielou by site
# Kruskal-Wallis chi-squared = 21, df = 12, p-value = 0.05

```

### Do the same for the Beta diversity instead

```{r beta diversity, fig.height=4}
# Prepare the community matrix to compute the beta diversity
dbeta <- unique(select(filter(d, !is.na(species)), site, species))
dbeta <- dcast(dbeta, site~species, fill = 0)
head(dbeta)
colsites <- dbeta[,1]
# dbeta <- replace(dbeta[,2:72], dbeta[,2:72]!="0", 1)
# Summer
dbeta <- replace(dbeta[,2:66], dbeta[,2:66]!="0", 1)
dbeta[,] <- sapply(dbeta[,],as.numeric)
rownames(dbeta) <- colsites
head(dbeta)
...
m <- betadiver(dbeta, method="w")
River_location <- betadisper(m, group=sites$rhone)
anova(River_location)
plot(River_location)

typo <- betadisper(m, group=sites$topography)
anova(typo)
plot(typo)

reg <- betadisper(m, group=sites$region)
anova(reg)
plot(reg)

mpa <- betadisper(m, group=sites$protection)
anova(mpa)
plot(mpa)

# SUMMER
# no signif diff either but nice differenciation betweet W and E side of the Rhone
...
cut <- 7 # ward.D2 cut at 5, but cutting at 6 enables to discriminate Vlfr from Corsica on Axe 4
...
# Plot the dendrogram
cols <- colorRampPalette(brewer.pal(8, "Paired"))
cols <- cols(8)
myPal <- c( "grey60", cols)
...
ggplot() + 
   ...
   geom_text(data = label(dendr), aes(x, y, label = label, colour = factor(cluster)), 
       hjust = -0.2, size = 4, show.legend = FALSE) +
   geom_hline(aes(yintercept=2.3), linetype=2)+
...
# Save plot
ggsave("figures/dendrogram_sites_SUMMER.pdf", width = 10, height = 3)


# Try to find cutoff with some criterion
NbClust(data=ddm, diss=dist_matrix, distance=NULL, min.nc=2, max.nc=9, method="ward.D2", index=c("kl", "ch", "cindex", "db", "silhouette", "duda", "pseudot2", "beale", "gap"))
# 3 clusters seems to be the best cutoff, but Vlfr with Bastia and Leucate --> due to higher sampling effort=higher diversity ?
...
labs <- str_c(dims, " (",round(ca$eig[1:4,2]),"%)")

ca_df <- arrange(ca_df, group)
# ca_df$label[which(ca_df$type=="col")] <- paste("italic('", ca_df$label[which(ca_df$type=="col")], "')", sep="")
# ca_df$label[which(ca_df$type=="Port Vendres")] <- "Port-Vendres"
# ca_df$label[which(ca_df$type=="La Ciotat")] <- "La_Ciotat"
# ca_df$label[which(ca_df$type=="Les Embiez")] <- "Les_Embiez"
# ca_df$label[which(ca_df$type=="Saint Florent")] <- "Saint-Florent"

# plot result
p1 <- ggplot(ca_df, aes(x=Dim1, y=Dim2, colour=group)) +
  scale_colour_manual(guide="none", values = myPal[2:9], na.value="grey50") +
  geom_point(size=1) +
  ggrepel::geom_text_repel(aes(label=label), size=4.2, segment.alpha=0.8) +
  scale_x_continuous(breaks=0) + scale_y_continuous(breaks=0) +
  labs(x=labs[1], y=labs[2]) + theme_light(base_size=14)
p2 <- ggplot(ca_df, aes(x=Dim3, y=Dim4, colour=group)) +
  scale_colour_manual(guide="none", values = myPal[2:9], na.value="grey50") +
  geom_point(size=1) +
  ggrepel::geom_text_repel(aes(label=label), size=4.2, segment.alpha=0.8) +
  scale_x_continuous(breaks=0) + scale_y_continuous(breaks=0) +
  labs(x=labs[3], y=labs[4]) + theme_light(base_size=14)

# plot and save graph
pdf(file = "figures/CA_species_sites_SUMMER.pdf", width = 10, height = 8)
...
# SUMMER: same results, higher signif
# Number of permutations: 999
# 
# Terms added sequentially (first to last)
# 
#            Df SumsOfSqs MeanSqs F.Model    R2 Pr(>F)   
# rhone       1      15.4   15.37    3.80 0.251  0.003 **
# topography  1       5.5    5.48    1.36 0.089  0.211   
# Residuals  10      40.4    4.04         0.660          
# Total      12      61.3                 1.000          
```
...
ggplot(filter(dp, ! site == "Carry")) +
  geom_vline(xintercept=0, size=1, colour="grey70") +
  #geom_histogram(aes(x=lag), binwidth=1) +
  geom_density(aes(x=lag, y=..count..), adjust=0.5, fill='white') +
  facet_wrap(~site, ncol=3) + theme_light(base_size = 14) + 
  scale_x_continuous("Lag (in days)") + scale_y_continuous("Density", limits=c(0, 1.3), expand=c(0,0))

# Save figure
# ggsave("figures/Fig9_spatial_lag_sites_density.pdf", width = 8, height = 7)
...
glance(wilcox.test(filter(dp, rhone=="West of Rhone")$lag, filter(dp, rhone=="East of Rhone")$lag, alternative = "g")) %>% select(statistic, p.value)
#   statistic p.value
# 1      1130  0.0028

# SUMMER: now non significant results
...
# Test for differences between Topography directly
glance(wilcox.test(filter(dp, topography=="Gulf of Lion")$lag, filter(dp, topography=="Ligurian sea")$lag, alternative = "two.sided")) %>% select(statistic, p.value)
#   statistic p.value
# 1      1263    0.23
...
sum(data.frame(filter(d, species == "Anguilla anguilla"))$n) # 45 

# Dorades
data.frame(filter(d, species == "Sparus aurata"))
sum(data.frame(filter(d, species == "Sparus aurata"))$n)
