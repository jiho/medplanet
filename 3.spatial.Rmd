...
ggplot(d_abund) + geom_boxplot(aes(x=sp, y=cpue), na.rm=TRUE) + facet_wrap(~topography) + ylim(0,10) + theme(axis.text.x=element_text(angle=35, hjust=1))
...
```{r}
# and the corresponding plot...
ggplot(d_abund) + geom_boxplot(aes(x=sp, y=cpue), na.rm=TRUE) + facet_wrap(~region) + ylim(0,10) + theme(axis.text.x=element_text(angle=35, hjust=1))
# TODO use violins + log transform, as in taxonomic composition
```
...
```{r}
data <- cast(dd, site~species, value = "mean")
# TODO cleanup code and use as.multivar
data.env <- left_join(data, sites, by=c("site"))
data.env <- select(data.env, site, topography, region)
data <- as.multivar(dd, "site")

adonis(data~region, data.env, permutations = 999, method="bray")$aov.tab
