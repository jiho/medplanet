...
ggplot(divt) + geom_boxplot(aes(x=site, y=value), na.rm=T) + facet_wrap(~index, ncol=1, scales="free_y") + theme(axis.text.x=element_text(angle=35, hjust=1))
...
#pairwise.wilcox.test(div$pielou, div$site, p.adjust.method = "holm")
...
dd <- d %>% filter(species %in% interest_species) %>% group_by(date, year, yday, site, species) %>% summarise(cpue=sum(cpue)) %>% ungroup()

# for winter species, shift the time
dd$yday[dd$species %in% c("Sarpa salpa", "Pagellus acarne")] <- (dd$yday[dd$species %in% c("Sarpa salpa", "Pagellus acarne")] + 183) %% 366
...
ggplot(dd) + facet_wrap(~species, scales="free_y", ncol=3) + theme(legend.position="bottom") +
...
ggplot(dp) +
  geom_vline(xintercept=0, size=2, colour="white") +
...
TODO commented out, move to temporal.
