...
pw <- suppressMessages(pairwise.aovq(dd$cpue, dd$year, tau=c(0.25, 0.5, 0.75, 0.9), R=100))
# plot
...
```{r fig.height=5}
dd <- d0 %>% filter(year != 2016, species %in% abundant_species) %>% group_by(year, species) %>% summarise(tot=sum(cpue), mean=mean(cpue), sd=sd(cpue), median=median(cpue), mad=mad(cpue))
...
However, years 2012 2013 2014 have the same (or very similar) sampling effort (in terms of sites sampled).

2012 : All sites except La Ciotat, Cassis, Carry, Les Embeiz
