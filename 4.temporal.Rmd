...
dd <- filter(dd, cpue < 200)

dd$days <- as.numeric((dd$date - min(dd$date)) / (24*3600))

q <- c(.25, .5, .75, .9)
rq_fit <- srq(dd$days, dd$cpue, tau=q, interval="confidence", df=30, se="boot")
rq_fit$date <- min(dd$date) + rq_fit$x * 24 * 3600
rq_fit$lower[rq_fit$lower < 0] <- 0
ggplot() +
  geom_point(aes(x=date, y=cpue), data=dd) +
  geom_ribbon(aes(x=date, ymin=lower, ymax=higher, fill=tau), data=rq_fit, alpha=0.3) +
  geom_line(aes(x=date, y=fit,  colour=tau), data=rq_fit)

rq_fit <- llrq(dd$days, dd$cpue, tau=q, bw=20, n=200, interval="confidence", se="boot")
rq_fit$date <- min(dd$date) + rq_fit$x * 24 * 3600
rq_fit$lower[rq_fit$lower < 0] <- 0

ggplot() +
  geom_point(aes(x=date, y=cpue), data=dd) +
  geom_ribbon(aes(x=date, ymin=lower, ymax=higher, fill=tau), data=rq_fit, alpha=0.2, colour=alpha("black", 0.1), size=0.25) +
  geom_line(aes(x=date, y=fit,  colour=tau), data=rq_fit)
```
...
TODO CA to delinate assemblages (potentially per season).
...
Pairwise wilcoxon test pvalues
```{r warning=FALSE}
dd <- d0 %>% filter(year != 2016) %>% group_by(date, year, site) %>% summarise(cpue=sum(cpue))

test <- kruskal.test(cpue ~ year, data=dd)
test
p_test<-pairwise.wilcox.test(dd$cpue,dd$year, p.adjust.method="bonferroni")
as.data.frame(p_test$p.value)

dd %>% group_by(year) %>% sum_tbl("cpue") %>% select(-n, -n_NA)
```

Replace kruskal by aov.rq
