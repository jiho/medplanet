...
# Try with srq_fit
#================
dd <- d0m %>% group_by(moon_phase, year, site, species) %>% summarise(cpue=sum(cpue))
q <- c(.25, .5, .75, .9)

srq_fit <- ddply(dd, ~species, function(x) {
  x <- srq(x$moon_phase, x$cpue, tau=q, interval="confidence", df=30, se="boot", R=100)
  x
}, .progress="text")


srq_fit$moon_phase <- srq_fit$x
srq_fit$lower[srq_fit$lower < 0] <- 0

ggplot(srq_fit) +
  geom_point(aes(x=moon_phase, y=cpue), data=filter(dd, cpue < 100), size=1, alpha=0.5, shape=16) +
  geom_ribbon(aes(x=moon_phase, ymin=lower, ymax=higher, fill=tau, alpha=tau)) +
  scale_alpha_manual(values=c(0., 0.2, 0., 0.2)) +
  geom_line(aes(x=moon_phase, y=fit,colour=tau)) +
  facet_wrap(~species, ncol=3, scales="free_y") + moon_x + no_y + labs(y="cpue (scaled per species)")
#============

ggplot(filter(d0m, species %in% abundant_species)) + geom_histogram(aes(x=moon_phase, weight=cpue, y=..ncount..), binwidth=0.05, fill="grey30", colour=NA) + facet_wrap(~species, ncol=3) + moon_x + no_y + labs(y="total number of larvae (scaled per species)")
...
TODO instead of polynomial fits, use local linear quantile regression llrq
...
TODO review if quantile regression is correctly done here

```{r fig.height=4}
suppressPackageStartupMessages(library("quantreg", quietly=TRUE))

# select data: common species, in summer, without extreme Spicara
dd <- filter(d0m, species %in% abundant_species, in_summer(date), cpue < 250)
...
# WEIGHT per effort around lunar phase
# select data: common species, in summer, without extreme Spicara
dd <- filter(d0m, species %in% abundant_species, in_summer(date), cpue < 250)
...
ggplot(dd, aes(x = moon_phase)) + geom_histogram(binwidth = 0.01)

# compute effort per moonphase
ef_moon <- data.frame(table(round(dd$moon_phase, 2)))
ef_moon <- dplyr::rename(ef_moon, moon_phase = Var1, effort = Freq)
ef_moon$moon_phase <- as.numeric(as.character(ef_moon$moon_phase))
ef_moon$effort <- ef_moon$effort / sum(ef_moon$effort)
dd$moon_phase <- round(dd$moon_phase, 2)
str(dd)
str(ef_moon)


# Join with effort (% of total effort per moon phase)
dd <- left_join(dd, ef_moon, by = "moon_phase")
...
qpred <- ldply(quantiles, function(q) {
  # quantile regression on log(n+1)
  m <- rq(log1p(cpue / effort) ~ moon_phase, tau=q, data=dd)
...
# plot the result
ggplot(mapping=aes(x=moon_phase)) +
  geom_point(aes(y=cpue/effort), data=dd, alpha=0.5) +
  geom_ribbon(aes(ymin=lower, ymax=higher, fill=quantile), data=qpred, alpha=0.3) +
  geom_path(aes(y=fit, colour=quantile), data=qpred) +
  # scale_colour_brewer(palette="Reds") + scale_fill_brewer(palette="Reds") +
  scale_y_continuous("cpue (in summer, log(n+1) transformed)", trans="log1p") +
...
system.time(llrq_fit <- llrq(dd$fake_date, dd$cpue, tau=q, bw=20, n=200, .parallel=T, .progress="text", smooth=1, interval="conf", se="boot", bsmethod="xy", R=1000))
stopImplicitCluster()

llrq_fit$fake_date <- llrq_fit$x 
llrq_fit$lower[llrq_fit$lower < 0] <- 0
...
pw <- ldply(pw, function(x) {
  x <- melt(x)
  x$X1 <- factor(x$X1, levels=levels(dd$year))
  x$X2 <- factor(x$X2, levels=levels(dd$year))
  x$signif <- x$value < 0.05
  return(x)
})
ggplot(pw) + geom_point(aes(X1, X2, shape=signif, colour=signif), na.rm=T) + theme(axis.text.x = element_text(angle=30, hjust=1), axis.title=element_blank()) + facet_wrap(~.id, ncol=1)
```
