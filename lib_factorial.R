...
as.multivar <- function(x, x.var, y.var="species", value="mean") {
  xd <- reshape2::dcast(data=x, as.formula(stringr::str_c(x.var, "~", y.var)), value.var=value, fill=0)
  rownames(xd) <- xd[,x.var]
  xd <- dplyr::select_(xd, stringr::str_c("-", x.var))
  return(xd)
}
...
augment.CA <- function(x, dimensions=c(1,2,3,4)) {
  d <- plyr::ldply(c("row", "col"), function(type) {
    ...
    names(d) <- stringr::str_c("Dim", dimensions)
    d$type <- type
    d$label <- row.names(d)
...
autoplot.CA <- function(x, dimensions=c(1,2), size=1, ...) {
  ...
  eig <- tidy(x)
  dims <- names(dat)[stringr::str_detect(names(dat), "Dim")]
  labs <- stringr::str_c(dims, " (",round(eig$prop.variance[dimensions]*100),"%)")
